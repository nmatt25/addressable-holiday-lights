...
const char *mqtt_client_name = "Holidaylights"; // Client connections can't have the same connection name
...
void callback(char* topic, byte* payload, unsigned int length) 
{
  ...
  if (newTopic == "holidayLights/configure")
  {
    ...
    if(effect == "Color_Chase" || effect == "Blocked_Colors" || effect == "Christmas_Chase" || effect == "Christmas_Blocks")
    {
    ...
    if(effect == "White_Twinkle" || effect == "Blue_Twinkle" || effect == "Christmas_Blue_Twinkle" || effect == "Christmas_White_Twinkle")
    {
...
void choosePattern()
{
  if(showLights == true)
  {
   ...
   if(effect == "Christmas_Blue_Twinkle")
   {
      firstblueXmas();
      secondblueXmas();
      wreathblueXmas();
   ...
   if(effect == "Christmas_White_Twinkle")
   {
      firstwhiteXmas();
      secondwhiteXmas();
      wreathwhiteXmas();
...
void setup() {
  ...
  ArduinoOTA.setHostname("HolidayLights");
  ArduinoOTA.begin(); 
...
/****************** BLOCKED Miami Dohlphins ********************/
void firstmiamiBlocked()
{
  FastLED.setBrightness(128);
  for( int mark = 0; mark < FIRSTFLOOR_LEDS; mark+=30)
  {
    for( int i = 0; i < 10; i++) 
    {
      if(startPosition == 0)
      {
      firstFloor[i+mark] = CRGB(150,50,0);
      firstFloor[i+10+mark] = CRGB(150,150,150);
      firstFloor[i+20+mark] = CRGB(0,180,150);
      }      
    }
  }
}

void secondmiamiBlocked()
{
  FastLED.setBrightness(128);
  for( int mark = 0; mark < SECONDFLOOR_LEDS; mark+=30)
  {
    for( int i = 0; i < 10; i++) 
    {
      if(startPosition == 0)
      {
      secondFloor[i+mark] = CRGB(150,50,0);
      secondFloor[i+10+mark] = CRGB(150,150,150);
      secondFloor[i+20+mark] = CRGB(0,180,150);
      }
    }
  }
}
...
/******************** TWINKLE *************************************/
/******************** Blue with White TWINKLE *************************************/
void firstblueTwinkle()
{
  fadeToBlackBy( firstFloor, FIRSTFLOOR_LEDS, 80);
  for( int i = 0; i < FIRSTFLOOR_LEDS-3; i+=3)
  {
...
void secondblueTwinkle()
{
  fadeToBlackBy( secondFloor, SECONDFLOOR_LEDS, 80);
  for( int i = 0; i < SECONDFLOOR_LEDS-3; i+=3)
  {
    if (random8() > twinkleChance)
    {
    ...
    {
      secondFloor[i] = CRGB( 000, 000, 255);
    }  
  }                     
}

void canesblueTwinkle()
{
...
void firstXmas1()
{
  if(startPosition < 6)
  {
    ...
    for(int pixel = startPosition+1; pixel < FIRSTFLOOR_LEDS; pixel+=6)  
    {
      firstFloor[pixel] = CRGB(200,000,000);
    }
    for(int pixel = startPosition+3; pixel < FIRSTFLOOR_LEDS; pixel+=6)
    {
      firstFloor[pixel] = CRGB(000,200,000);
    }
    for(int pixel = startPosition+5; pixel < FIRSTFLOOR_LEDS; pixel+=6)
    {
      firstFloor[pixel] = CRGB(000,000,200);
    }
  }
  if(startPosition == 1)
  {
    firstFloor[0]=CRGB(000,000,200);
  }
  if(startPosition == 2)
  {
    firstFloor[1]=CRGB(000,000,200);
  }
  if(startPosition == 3)
  {
    firstFloor[2]=CRGB(000,000,200);
    firstFloor[0]=CRGB(000,200,000);
  }
  if(startPosition == 4)
  {
    firstFloor[3]=CRGB(000,000,200);
    firstFloor[1]=CRGB(000,200,000);
  }
  if(startPosition == 5)
  {
    firstFloor[4]=CRGB(000,000,200);
    firstFloor[2]=CRGB(000,200,000);
    firstFloor[0]=CRGB(200,000,000);
  }
}
...
void secondXmas1()
{
  if(startPosition < 6)
  {
    ...
    for(int pixel = startPosition+1; pixel < SECONDFLOOR_LEDS; pixel+=6) 
    {
      secondFloor[pixel] = CRGB(200,000,000);
    }
    for(int pixel = startPosition+3; pixel < SECONDFLOOR_LEDS; pixel+=6)  
    {
      secondFloor[pixel] = CRGB(000,200,000);
    }
    for(int pixel = startPosition+5; pixel < SECONDFLOOR_LEDS; pixel+=6) 
    {
      secondFloor[pixel] = CRGB(000,000,200);
    }
  }
  if(startPosition == 1)
  {
    secondFloor[0]=CRGB(000,000,200);
  }
  if(startPosition == 2)
  {
    secondFloor[1]=CRGB(000,000,200);
  }
  if(startPosition == 3)
  {
    secondFloor[2]=CRGB(000,000,200);
    secondFloor[0]=CRGB(000,200,000);
  }
  if(startPosition == 4)
  {
    secondFloor[3]=CRGB(000,000,200);
    secondFloor[1]=CRGB(000,200,000);
  }
  if(startPosition == 5)
  {
    secondFloor[4]=CRGB(000,000,200);
    secondFloor[2]=CRGB(000,200,000);
    secondFloor[0]=CRGB(200,000,000);
  }
}
...
void canesXmas1()
{
  ...
  if(startPosition == 1 || startPosition == 3 || startPosition == 5 )
  {
    for(int pixel = 1; pixel < 176; pixel+=2)
    {
      candyCanes[pixel] = CRGB(200,200,200);
    }
  }
}
...
void firstXmas2()
{
  ...
  for( int mark = 0; mark < FIRSTFLOOR_LEDS; mark+=30)
  {
    for( int i = 0; i < 9; i++) 
    {
      if(startPosition == 0 || startPosition == 1)
      {
      firstFloor[i+mark] = CRGB(200,000,000);
      firstFloor[i+10+mark] = CRGB(000,200,000);
      firstFloor[i+20+mark] = CRGB(000,000,200);
      }
      if(startPosition == 2 || startPosition == 3)
      {
      firstFloor[i+mark] = CRGB(000,200,000);
      firstFloor[i+10+mark] = CRGB(000,000,200);
      firstFloor[i+20+mark] = CRGB(200,000,000);
      }
      if(startPosition == 4 || startPosition == 5)
      {
      firstFloor[i+mark] = CRGB(000,000,200);
      firstFloor[i+10+mark] = CRGB(200,000,000);
      firstFloor[i+20+mark] = CRGB(000,200,000);
      }
    }
  }
}
...
void secondXmas2()
{
  ...
  for( int mark = 0; mark < SECONDFLOOR_LEDS; mark+=30)
  {
    for( int i = 0; i < 9; i++) 
    {
      if(startPosition == 0 || startPosition == 1)
      {
      secondFloor[i+mark] = CRGB(200,000,000);
      secondFloor[i+10+mark] = CRGB(000,200,000);
      secondFloor[i+20+mark] = CRGB(000,000,200);
      }
      if(startPosition == 2 || startPosition == 3)
      {
      secondFloor[i+mark] = CRGB(000,200,000);
      secondFloor[i+10+mark] = CRGB(000,000,200);
      secondFloor[i+20+mark] = CRGB(200,000,000);
      }
      if(startPosition == 4 || startPosition == 5)
      {
      secondFloor[i+mark] = CRGB(000,000,200);
      secondFloor[i+10+mark] = CRGB(200,000,000);
      secondFloor[i+20+mark] = CRGB(000,200,000);
      }
    }
  }
}
...
void canesXmas2()
{
  ...
  if(startPosition == 0 || startPosition == 1)
  {
    for(int cane = 0; cane < 176; cane+=33)
    {
      for(int pixel = 0; pixel < 10; pixel++)
      {
        candyCanes[cane+pixel] = CRGB(200,0,0);
      }  
    }
  }
  if(startPosition == 2 || startPosition == 3)
  {
    for(int cane = 11; cane < 176; cane+=33)
    {
      for(int pixel = 0; pixel < 10; pixel++)
      {
        candyCanes[cane+pixel] = CRGB(200,200,200);
      }  
    }
  }
  if(startPosition == 4 || startPosition == 5)
...
{
  fadeToBlackBy( firstFloor, FIRSTFLOOR_LEDS, 80);
  for( int i = 0; i < FIRSTFLOOR_LEDS-2; i+=2)
  {
    if (random8() > twinkleChance)
    {
      firstFloor[i] = CRGB( 255, 255, 255);
...
{
  fadeToBlackBy( secondFloor, SECONDFLOOR_LEDS, 80);
  for( int i = 0; i < SECONDFLOOR_LEDS-2; i+=2)
  {
    if (random8() > twinkleChance)
    {
      secondFloor[i] = CRGB( 255, 255, 255);
    ...
    {
      secondFloor[i] = CRGB( 000, 000, 200);
    }  
  }                     
}
...
void canesblueXmas()
{
  fadeToBlackBy( candyCanes, CANDYCANE_LEDS, 80);
  for( int i = 0; i < CANDYCANE_LEDS-1; i+=1)
  {
    if(random8() > twinkleChance)
    {
      candyCanes[i]=CRGB(255,255,255);
    }
    else
    {
      candyCanes[i]=CRGB(200,0,0);
    }
  }
}
...
void wreathblueXmas()
{
  fadeToBlackBy( wreath, WREATH_LEDS, 80);
  for( int i = 0; i < WREATH_LEDS-2; i+=2)
  {
    if(random8() > twinkleChance)
    {
      wreath[i]=CRGB(000,000,255);
    ...
    {
      wreath[i]=CRGB(100, 100, 100);
    }
  }
}
/******************* Christmas White Twinkle **********************************/
void firstwhiteXmas()
{
  fadeToBlackBy( firstFloor, FIRSTFLOOR_LEDS, 80);
  for( int i = 0; i < FIRSTFLOOR_LEDS-2; i+=2)
  {
    if (random8() > twinkleChance)
    {
      firstFloor[i] = CRGB( 255, 000, 255);
    }
    else
    {
      firstFloor[i] = CRGB( 100, 100, 100);
    }  
  }                     
}
...
void secondwhiteXmas()
{
  ...
  for( int i = 0; i < SECONDFLOOR_LEDS-2; i+=2)
  {
    if (random8() > twinkleChance)
    {
      secondFloor[i] = CRGB( 255, 000, 255);
    }
    else
    {
      secondFloor[i] = CRGB( 100, 100, 100);
    }  
  }                     
}
...
void solid()
{
  if(startPosition == 0)
  {
    fill_solid(secondFloor, SECONDFLOOR_LEDS, CRGB(red2,green2,blue2));
